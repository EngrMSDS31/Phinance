...
async function recalcHolding(holdingId: number) {
  ...
  if (holding && FUNDS_BONDS_MARKETS.has(holding.market)) {
...
router.post("/", async (req, res) => {
  try {
    ...
    // Auto-create dividend calendar entry for dividend transactions
    if (type === "DIVIDEND" && resolvedHoldingId) {
      const [holdingRow] = await db.select().from(holdingsTable).where(eq(holdingsTable.id, resolvedHoldingId));
      if (holdingRow) {
        await db.insert(dividendEventsTable).values({
          portfolioId, holdingId: resolvedHoldingId, symbol: holdingRow.symbol, name: holdingRow.name,
          dividendType: "ORDINARY", exDate: date,
          totalAmount: String(Math.abs(parseFloat(amount ?? 0))),
          currency: txCurrency,
        }).onConflictDoNothing();
      }
    }
