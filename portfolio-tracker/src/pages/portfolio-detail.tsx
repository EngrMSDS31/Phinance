            ...
            <div className="space-y-1.5">
              ...
              <Select value={txType} onValueChange={v => { setTxType(v); setPriceStatus(null); }}>
                ...
                <SelectContent>
                  ...
                  {isBondFund && (
                    <>
                      <div className="px-2 pt-1.5 pb-0.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground border-t border-border/50 mt-0.5">Funds / Bonds</div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={txTransferHoldingId} onValueChange={setTxTransferHoldingId}>
                  ...
                  <SelectContent>
                    {(holdings ?? [])
                      .filter((h: any) => h.market === "FUNDS" || h.market === "BONDS")
                      .map((h: any) => (
                    ...
                    {(holdings ?? []).filter((h: any) => h.market === "FUNDS" || h.market === "BONDS").length === 0 && (
                      <div className="px-3 py-2 text-xs text-muted-foreground">No FUNDS/BONDS holdings yet — add one first</div>
                    )}
                  </SelectContent>
                </Select>
              </div>
                  ...
                  <TableBody>
                    {holdingTxs.map(t => (
                      <TableRow key={t.id}>
                        ...
                        <TableCell className="text-right font-mono text-xs font-medium">{formatCurrency(t.amount, t.currency)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
...
export default function PortfolioDetail() {
  ...
  const onSubmit = (values: z.infer<typeof holdingSchema>) => {
    ...
    else if (values.assetType === "FUND" || portfolio?.type === "FUNDS") market = "FUNDS";
    else if (values.assetType === "BOND" || portfolio?.type === "BONDS") market = "BONDS";
    else if (["SAVINGS", "CASH_ASSET", "CUSTOM"].includes(values.assetType)) market = "CUSTOM";
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="currency" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="USD" {...field} className="uppercase" onChange={e => field.onChange(e.target.value.toUpperCase())} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
      ...
      <Card className="border-border/60">
        <CardContent className="pt-3 pb-3 md:pt-5 md:pb-5">
          ...
          <div className="text-2xl md:text-4xl font-bold tracking-tight">
            {formatCurrency(
              portfolioEngine?.totalPortfolioValue ?? (summary?.totalValue || 0) + (summary?.cashBalance || 0),
              portfolio.baseCurrency
            )}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
            Invested: {formatCurrency(portfolioEngine?.totalInvested ?? summary?.netDeposited ?? 0, portfolio.baseCurrency)}
          </div>
        </CardContent>
      </Card>
      <div className="grid grid-cols-3 gap-2 md:gap-4">
        <Card><CardContent className="p-2.5 md:pt-4 md:pb-4 md:px-4">
          ...
          <div className={`text-sm md:text-xl font-bold mt-1 font-mono ${cnValue(portfolioEngine?.totalProfit ?? summary?.totalGains ?? 0)}`}>
            {formatCurrency(portfolioEngine?.totalProfit ?? summary?.totalGains ?? summary?.unrealizedGain ?? 0, portfolio.baseCurrency)}
          </div>
        ...
        <Card><CardContent className="p-2.5 md:pt-4 md:pb-4 md:px-4">
          ...
          <div className="text-sm md:text-xl font-bold mt-1 font-mono">{formatCurrency(portfolioEngine?.totalCurrentValue ?? summary?.totalValue ?? 0, portfolio.baseCurrency)}</div>
        ...
        <Card><CardContent className="p-2.5 md:pt-4 md:pb-4 md:px-4">
          ...
          <div className="text-sm md:text-xl font-bold mt-1 font-mono">{formatCurrency(summary?.cashBalance || 0, portfolio.baseCurrency)}</div>
              ...
              filteredSortedHoldings.map(h => (
                ...
                >
                  ...
                  <div
                    ...
                    onTouchMove={e => {
                      if (holdingTouchStartRef.current?.id === h.id)
                        holdingTouchDeltaRef.current = e.touches[0].clientX - holdingTouchStartRef.current.x;
                    }}
                    onTouchEnd={() => {
