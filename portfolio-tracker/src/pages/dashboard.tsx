...
import {
  ...
  useGetUpcomingDividends,
  useListPortfolios,
  useListHoldings,
  useCreateTransaction,
  useSearchSymbols,
  getGetDashboardSummaryQueryKey,
  getGetDashboardAllocationQueryKey,
  ...
  return (
    <Card className={highlight ? "md:ring-1 md:ring-primary/20" : ""}>
      <CardContent className="p-4 md:pt-8 md:px-6 md:pb-8">
        <div className="text-[10px] md:text-[13px] text-muted-foreground uppercase tracking-wider font-semibold leading-none">
          {title}
        </div>
        {loading ? (
          <Skeleton className="h-6 md:h-8 w-24 md:w-36 mt-2" />
        ) : (
          <div className={`text-lg md:text-2xl font-bold leading-tight mt-2 ${valueClass ?? ""}`}>
            {value}
          </div>
        )}
...
function DashboardAddTransactionDialog() {
  ...
  const portfolios = toArray<any>(portfoliosData);
  const holdings = toArray<any>(holdingsData);
  ...
  const searchMarket =
    selectedPortfolio?.type && selectedPortfolio.type !== "MIXED"
      ? selectedPortfolio.type
      : undefined;

  const { data: searchResultsData, isFetching: isSearching } = useSearchSymbols(
  ...
  const searchResults = toArray<any>(searchResultsData);
  ...
  const existingSet = new Set(
    existingMatches.map((h: any) => `${h.symbol}:${h.market}`)
  );

  const liveSuggestions = searchResults.filter(
    (r: any) => !existingSet.has(`${r.symbol}:${r.market}`)
  );
  ...
  const fetchLivePrice = async (sym: DashSymbol) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/prices?symbols=${sym.symbol}:${sym.market}`, {
        credentials: "include",
      });
  ...
  const handleSelectSymbol = (sym: DashSymbol) => {
    ...
    setSymbolQuery("");
    setShowSuggestions(false);
    setTxCurrency(selectedPortfolio?.baseCurrency || "USD");
    setPriceStatus(null);
    setPriceLabel(null);
  ...
  }, [subtotal, open, step, isTrade, isIncome, portfolioFeeRate, portfolioTaxRate, txFeeAmt, txTaxAmt]);
  ...
  const handleSubmit = () => {
    if (needsHolding && !selectedSymbol) {
      toast({ title: "Select a stock or symbol", variant: "destructive" });
      return;
    }

    const amt = totalAmount || pricePerShare || 0;
    if (amt <= 0) {
      toast({ title: "Enter an amount or qty + price", variant: "destructive" });
      return;
    }
    ...
    createTx.mutate(
      { portfolioId: selectedPortfolioId!, data: payload as any },
      {
        onSuccess: () => {
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <div className="px-2 pt-1.5 pb-0.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground border-t border-border/50 mt-0.5">
                        Cash
                      </div>
                ...
                <div className="space-y-1.5" ref={searchRef}>
                  ...
                  {selectedSymbol ? (
                    <div className="flex items-center gap-2 px-3 py-2 rounded-md border border-border bg-muted/30">
                      ...
                      <span className="text-muted-foreground text-xs ml-1 truncate flex-1">
                        {selectedSymbol.name}
                      </span>
                    ...
                    <div className="relative">
                      ...
                      {isSearching && (
                        <RefreshCw className="absolute right-2.5 top-2.5 w-3.5 h-3.5 text-muted-foreground animate-spin pointer-events-none" />
                      )}
                      ...
                      {showSuggestions && (existingMatches.length > 0 || liveSuggestions.length > 0) && (
                        <div className="absolute z-50 top-full mt-1 w-full bg-popover border border-border rounded-md shadow-lg max-h-56 overflow-y-auto">
                          {existingMatches.length > 0 && (
                            <>
                              <div className="px-2.5 py-1 text-xs text-muted-foreground bg-muted/40 border-b border-border">
                                In this portfolio
                              </div>
                          ...
                          {liveSuggestions.length > 0 && (
                            <>
                              ...
                              {liveSuggestions.map((s: any, i: number) => (
                                <button
                                  key={`live-${i}`}
                                  className="w-full text-left px-3 py-2 hover:bg-muted/50 flex items-center gap-2"
                                  onMouseDown={(e) => {
                  ...
                  {priceStatus && !isFetchingPrice && (
                    ...
                    >
                      ...
                      {priceStatus === "unavailable"
                        ...
                        : priceLabel ?? (priceStatus === "live" ? "Live price" : "Last available price")}
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder="0.00"
                      value={txFeeAmt}
                      onChange={(e) => setTxFeeAmt(e.target.value)}
                      className="h-9 font-mono"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder="0.00"
                      value={txTaxAmt}
                      onChange={(e) => setTxTaxAmt(e.target.value)}
                      className="h-9 font-mono"
                    />
                  </div>
                </div>
              ...
              {totalAmount > 0 && (
                <div className="bg-muted/50 rounded p-3 text-sm flex justify-between">
                  ...
                  <span className="font-mono font-semibold">
                    {txCurrency} {totalAmount.toFixed(2)}
                  </span>
                </div>
              )}
...
export default function Dashboard() {
  ...
  const portfolios = toArray<any>(portfoliosData);
  const allocation = toArray<any>(allocationData);
  const upcomingDividends = toArray<any>(upcomingDividendsData);
  ...
  const allHoldingsQueries = useQueries({
    queries: allPortfolioIds.map((id) => ({
      ...
      queryFn: async () => {
        const r = await fetch(`${BASE_URL}/api/portfolios/${id}/holdings`, {
          credentials: "include",
        });
        if (!r.ok) return [];
        return r.json();
  ...
  const allTxQueries = useQueries({
    queries: allPortfolioIds.map((id) => ({
      ...
      queryFn: async () => {
        const r = await fetch(`${BASE_URL}/api/portfolios/${id}/transactions?limit=1000`, {
          credentials: "include",
        });
        if (!r.ok) return [];
        return r.json();
  ...
  const fifoEngine = useMemo(() => {
    ...
    allPortfolioIds.forEach((_, i) => {
      const pHoldings = toArray<any>(allHoldingsQueries[i]?.data);
      const pTxItems = toArray<any>(allTxQueries[i]?.data);
      const currency = portfolios[i]?.baseCurrency ?? "USD";

      const HOLDING_TX_TYPES = new Set([
        "BUY",
        "SELL",
        "DIVIDEND",
        "COUPON_INTEREST",
      ...
      pHoldings.forEach((h: any) => {
        const isFundBond = FUND_BOND_MARKETS.has(h.market);
  ...
  const fxAdjustedAllocation = useMemo(() => {
    if (allPortfolioIds.length === 0) return allocation;
    if (allHoldingsQueries.some((q) => q.isLoading) || allTxQueries.some((q) => q.isLoading)) {
      return allocation;
    }
    ...
    <div className="space-y-3 md:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
        ...
        <MetricCard
          ...
          subtitle={
            grandTotal > 0
              ? `${formatPercent((fifoEngine.totalCurrentValue / grandTotal) * 100)} of total`
              : undefined
          }
          loading={dashLoading}
        />
                    ...
                    <div className="relative w-full md:w-[55%] h-[180px] md:h-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          >
                            {allocWithWeight.map((entry: any, index: number) => (
                              <Cell
                                key={`cell-${index}`}
                                fill={entry.color}
                                opacity={
        ...
        <Card>
          <CardHeader className="pb-2 px-3 pt-3 md:px-6 md:pt-6">
            <div className="flex items-center justify-between">
              ...
              <Link href="/dividend-calendar" className="text-xs text-primary hover:underline">
                View all
              </Link>
            </div>
          </CardHeader>
                    ...
                    <div key={div.id ?? idx} className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground font-mono">
                          {div.paymentDate
                            ? format(new Date(div.paymentDate), "MMM d")
                            : div.exDate
                            ? `Ex: ${format(new Date(div.exDate), "MMM d")}`
                            : "—"}
                        </div>
                      </div>
                      <div className="text-right shrink-0">
