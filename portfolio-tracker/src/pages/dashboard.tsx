    ...
    <Card className={highlight ? "md:ring-1 md:ring-primary/20" : ""}>
      <CardContent className="p-4 md:pt-8 md:px-6 md:pb-8">
        ...
        {loading ? (
          ...
          <div className={`text-lg md:text-2xl font-bold leading-tight mt-2 ${valueClass ?? ""}`}>{value}</div>
        )}
        {subtitleNode && !loading && (
          <div className="text-[10px] md:text-xs mt-1 leading-tight">{subtitleNode}</div>
        )}
        {subtitle && !subtitleNode && !loading && (
          <div className={`text-[10px] md:text-xs mt-1 leading-tight ${subtitleClass ?? "text-muted-foreground"}`}>{subtitle}</div>
        )}
      </CardContent>
    </Card>
  );
  ...
  const fxAdjustedAllocation = useMemo(() => {
    ...
    return allPortfolioIds.map((_, i) => {
      ...
      const holdingEntries = pHoldings.map((h: any) => {
        const isFundBond = _FUND_BOND_MARKETS.has(h.market);
        const entry: any = {
