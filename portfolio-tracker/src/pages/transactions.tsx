    ...
    <Dialog open={open} onOpenChange={v => { setOpen(v); if (!v) resetForm(); }}>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          ...
          {totalAmount > 0 && (
            <div className="bg-muted/50 rounded p-3 text-sm flex justify-between items-center">
...
function EditTransactionDialog({
  ...
  const needsQtyPrice = ["BUY", "SELL"].includes(transaction.type);
  ...
  const netAmount = transaction.type === "BUY"
    ? grossVal + feeVal + taxVal
    : grossVal - feeVal - taxVal;
  const showNetSummary = grossVal > 0;

  // Auto-compute amount from qty × price for BUY/SELL
  useEffect(() => {
    if (needsQtyPrice) {
  ...
  const handleSave = () => {
    ...
    updateTx.mutate({
      ...
      data: {
        ...
        feeAmount: fee ? parseFloat(fee) : null,
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          <div className="flex items-center gap-2">
            <span className={`text-sm font-semibold ${typeTextClass(transaction.type)}`}>{formatTxType(transaction.type)}</span>
            {transaction.holdingSymbol && (
              <span className="text-sm font-medium font-mono text-foreground">{transaction.holdingSymbol}</span>
            )}
          </div>
          ...
          {needsQtyPrice && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-sm font-medium">Quantity</label>
                <Input type="number" min="0" step="any" value={qty} onChange={e => setQty(e.target.value)} className="h-9 font-mono" />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium">Price</label>
                <Input type="number" min="0" step="any" value={price} onChange={e => setPrice(e.target.value)} className="h-9 font-mono" />
              </div>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium">Amount</label>
            <Input type="number" min="0" step="any" value={amount} onChange={e => setAmount(e.target.value)} className="h-9 font-mono" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Fee</label>
              <Input type="number" min="0" step="0.0001" value={fee} onChange={e => setFee(e.target.value)} className="h-9 font-mono" />
            </div>
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Tax</label>
              <Input type="number" min="0" step="0.0001" value={tax} onChange={e => setTax(e.target.value)} className="h-9 font-mono" />
            </div>
          </div>

          <div className="space-y-1.5">
            <label className="text-sm font-medium">Currency</label>
            <Input value={currency} onChange={e => setCurrency(e.target.value.toUpperCase())} className="h-9 font-mono uppercase" />
          </div>
          ...
          {showNetSummary && (
            <div className="bg-muted/50 rounded p-3 text-sm space-y-1.5">
              <div className="flex justify-between">
                ...
                <span className="font-mono">{currency} {grossVal.toFixed(2)}</span>
              </div>
              {feeVal > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Fee</span>
                  <span className="font-mono text-muted-foreground">− {currency} {feeVal.toFixed(2)}</span>
                </div>
              )}
              {taxVal > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Tax</span>
                  <span className="font-mono text-muted-foreground">− {currency} {taxVal.toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between items-center pt-1 border-t border-border">
                <span className="font-semibold">Net Amount</span>
                <span className={`font-mono font-semibold ${amountClass(transaction.type)}`}>
                  {currency} {Math.max(0, netAmount).toFixed(2)}
                </span>
              </div>
            </div>
          )}
