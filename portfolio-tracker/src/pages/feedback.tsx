...
export default function Feedback() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent("Folio App Feedback");
    const body = encodeURIComponent(`${email ? `From: ${email}\n\n` : ""}${message}`);
    window.open(`mailto:feedback@folio.app?subject=${subject}&body=${body}`, "_blank");
    setSubmitted(true);
    setEmail("");
    setMessage("");
  };
  ...
  return (
    <div className="space-y-6 max-w-lg mx-auto">
      <div>
        ...
        <p className="text-xs md:text-sm text-muted-foreground mt-1">Your input helps make Folio better for everyone.</p>
      </div>
